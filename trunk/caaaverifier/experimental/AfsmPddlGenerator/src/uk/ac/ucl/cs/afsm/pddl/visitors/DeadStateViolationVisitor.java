 ...
 *
 */
public class DeadStateViolationVisitor implements AfsmVisitor {
	 ...
	 * 
	 */
	public DeadStateViolationVisitor(String folderName) {
	...
	public void visit(Afsm afsm) {
		...
		for (State s : afsm.states) {
			...
			File stateFile = new File(folder.getAbsolutePath() + "/DeadState-" + s.getName() + ".pddl");
			if (stateFile.exists()) {
				stateFile.delete();
			}
			try {
			...
			ps.print("; Dead state violation goal for state ");
			ps.print(s.getName());
			writeAlignment();
			ps.print("; If no solution is found then the state is unreachable ");
			writeAlignment();
