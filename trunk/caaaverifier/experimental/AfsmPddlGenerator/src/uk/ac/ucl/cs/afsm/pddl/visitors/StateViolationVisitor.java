...
import uk.ac.ucl.cs.afsm.pddl.State;
 ...
 *
 */
public class StateViolationVisitor implements AfsmVisitor {
	 ...
	 * 
	 */
	public StateViolationVisitor(String folderName) {
	...
	public void visit(Afsm afsm) {
		...
		for (State s : afsm.states) {
			...
			if (s.getInStateCondition() == null) {
				// No in state condition
				continue;
			}
			
			File stateFile = new File(folder.getAbsolutePath() + "/ViolateInStateConditions-" + s.getName() + ".pddl");
			if (stateFile.exists()) {
				stateFile.delete();
			}
			try {
			...
			ps.print("; State violation goal for state ");
			ps.print(s.getName());
			writeAlignment();
			ps.print("; If a solution is found then the precondition of that state have been violated");
			writeAlignment();
			
			ps.print("(define (problem violate-state-");
			ps.print(s.getName());
			ps.print(")");
			...
			alignment ++;
			writeAlignment();
			s.getInStateCondition().accept(this);
			alignment --;
			writeAlignment();
			ps.print(")");
